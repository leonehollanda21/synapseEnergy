...
def _extract_span_data(node, ctx):
    ...
    Returns:
        Tuple of (messages, models, model_settings)
    """
    # Extract models and settings from context
    model = None
    model_settings = None
    if hasattr(ctx, "deps"):
        model = getattr(ctx.deps, "models", None)
...
def _patch_graph_nodes():
    ...
    ModelRequestNode -> Creates ai_client span for models requests
